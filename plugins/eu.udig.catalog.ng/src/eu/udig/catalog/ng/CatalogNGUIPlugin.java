 ...
 * @since 1.2.0
 */
public class CatalogNGUIPlugin extends AbstractUdigUIPlugin {
    
    private static CatalogNGUIPlugin plugin;
    
    /**
     * The id of the plug-in
     */
    public static final String ID = "eu.udig.catalog.ng.ui"; //$NON-NLS-1$
...
//                     updateCache(event.getDelta(), p);
    ...
    public static Image image( IResolve resolve ) {
        ...
        if (resolve instanceof IResolveFolder ) {
            return CatalogNGUIPlugin.getDefault().getImage(ISharedImages.FOLDER_OBJ);
            ...
            return CatalogNGUIPlugin.getDefault().getImage( iconId );
            ...
            return CatalogNGUIPlugin.getDefault().getImage(ISharedImages.CATALOG_OBJ);
        }
        return CatalogNGUIPlugin.getDefault().getImage(ISharedImages.RESOURCE_OBJ);
    }
    ...
    public static ImageDescriptor icon( final IResolve resolve, IProgressMonitor monitor )
        ...
        if (resolve instanceof IResolveFolder) {
            ...
            return icon != null ? icon : CatalogNGUIPlugin.getDefault().getImageDescriptor(ISharedImages.FOLDER_OBJ);
        }

        if (resolve instanceof ICatalog)
            return CatalogNGUIPlugin.getDefault().getImageDescriptor(
    ...
    private static ImageDescriptor icon( IGeoResource resource, IProgressMonitor monitor )
        ...
        boolean isFeature = resource.canResolve(FeatureSource.class);
        String iconId = iconInternalResource( resource.getID(), isFeature );
        return CatalogNGUIPlugin.getDefault().getImageDescriptor( iconId );
    }
    ...
    private static ImageDescriptor icon( IResolveFolder folder, IProgressMonitor monitor )
        ...
        // check for default icon last
        return CatalogNGUIPlugin.getDefault().getImageDescriptor( ISharedImages.FOLDER_OBJ );
    }
    ...
    public static CatalogNGUIPlugin getDefault() {
        return plugin;
    }
